...
def fn(L, Nx, dt, T, w=1.):
    ...
    initialFunc[0] = 0.8/(1+np.exp((x-1)/0.25))
    q = initialFunc[:]
    R = []
    R.append(q.copy())
...
#        return 8*np.array([xy[0]*(1-xy[0]), A*xy[0]*(1-xy[0])-xy[1]])
...
def solveFN(Nx, Ny, ksi, q, ab, A):
    ...
    q0 = np.array([slin.solve(ab[0], side0[:,j]) for j in range(Ny+1)])
    q1 = np.array([slin.solve(ab[1], side1[:,j]) for j in range(Ny+1)])
    ...
    q0 = np.array([slin.solve(ab[0], side0[i,:]) for i in range(Nx+1)])
    q1 = np.array([slin.solve(ab[1], side1[i,:]) for i in range(Nx+1)])
    q = np.array((q0, q1))
    return q
...
#        A = 0.55 - 0.4*A
        ...
        A[0+(iteration//parA):10+(iteration//parA),:] = 0.1
#        A[0+(iteration//parA):10+(iteration//parA),:5] = -0.1
...
#    metadata = dict(title='heat dynamic', artist='Matplotlib')
    ...
    anim.save('FN_PBC_{}.mp4'.format(time.strftime("%Y%m%d-%H%M%S")), writer=writer)
#    plt.show()
#    plt.ion()
##############################################################################
    print(time.strftime("%a, %d %b %Y %H:%M:%S", time.localtime()))
