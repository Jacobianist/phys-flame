# -*- coding: utf-8 -*-
"""
Created on Wed Sep 27 16:39:13 2017
...
def fn(L, Nx, dt, T, w=1.):
    ...
    initialFunc[0] = 1/(1+np.exp((x-3)/0.5))
    q = initialFunc[:]
    R = []
    R.append(q.copy())
    ...
    ab = np.array([np.vstack((a[i], b[i], c[i])) for i in [0, 1]])
...
#    metadata = dict(title='heat dynamic', artist='Matplotlib')
    ...
    # anim.save('FNKPP_NBC_{}.mp4'.format(time.strftime("%Y%m%d-%H%M%S")), writer=writer)
    plt.show()
#    plt.ion()
###############################################################################
    print(time.strftime("%a, %d %b %Y %H:%M:%S", time.localtime()))
