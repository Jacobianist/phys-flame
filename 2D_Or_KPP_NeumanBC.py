...
###############################################################################
    ...
    rangeOf = fn(L, Nx, dt, T, 0.5)
    rangeOf = rangeOf[:, 1, :]
    print(np.min(rangeOf), np.max(rangeOf))
    rangeOf = 1+ rangeOf
###############################################################################
    initialFunc = np.zeros((2, Nx+1, Ny+1))
    initialFunc[0] = np.genfromtxt('d:\\python\\flame\\1or.out')[::2]
    initialFunc[1] = np.genfromtxt('d:\\python\\flame\\2or.out')[::2]
#    initialFunc[0] = .1/(1+np.exp((x-1)/0.25))
...
#    anim.save('OrKpp_NBC_{}.mp4'.format(time.strftime("%Y%m%d-%H%M%S")), writer=writer)
    plt.show()
#    plt.ion()
###############################################################################
    print(time.strftime("%a, %d %b %Y %H:%M:%S", time.localtime()))
